...
| rule | 法源 | 觸發 |
|---|---|---|
| `rest11h` | §34 | 相鄰兩班「有休息但不足」：間隔 **>0 且 <11h**（間隔=0 屬連續班，見下） |
| `continuous12h` | §32 | 相接(間隔=0)班次合併後**連續工時 >12h、中間無休息**（如大夜 `23-07` 直接接早班 `7-15`＝連續 16h）。2026-07-25 依實務新增，**軟擋**（可知情放行） |
| `weekly1off` | §36 | 連續出勤達 7 天（含跨週） |
| `daily12h` | §32 | 當日總時數（跨店加總）> 設定值（預設 12h） |
| `monthlyOt46` | §32 | 當月累計加班 > 設定值（預設 46h） |
| （硬擋）時間重疊、未滿18夜間 | §48 | 同一時刻雙排（物理不可能）／未滿18歲排 22:00–06:00 |

> **連續班 vs 休息不足的分界**：間隔=0（下班點＝下一班上班點）代表兩班相接、其實是**一整段連續班**，故不套 `rest11h`（否則 `7-11`＋`11-15` 這種正常 8h 分段會被誤報），改由 `continuous12h` 檢查合併後總長。間隔 >0 才是真正的「兩段休息之間不足 11h」。
